...
function getArtistByIndex(array, idx) {
  return `the artist at index ${idx} is ${array[idx]['name']}`;
}
// console.log('Task 3', getArtistByIndex(artists, 2));
// console.log('Task 3', getArtistByIndex(artists[2].name, artists[2].id));
...
function listOfNames(arrayParam) {
  const copyArray = [...arrayParam];
  for(let i = 0; i < arrayParam.length; i++) {
    copyArray[i] = arrayParam[i]['name'];
  }
  return copyArray;
}

// console.log('task 4', listOfNames(artists));
// console.log('task 4', listOfNames(artists['name']));
...
function removeArtist(arrayParam, num) {
  const copyArray = [...arrayParam];
...
function addArtist(arrayParam, nameParam, yearsParam, genreParam, nationalityParam, bioParam) {
  const arrayParamCopy = [...arrayParam];
  const newObj = {
    name: nameParam,
    years: yearsParam,
    genre: genreParam,
  ...
  arrayParamCopy.push(newObj);
  return arrayParamCopy;
}

// console.log('Task 6', addArtist(artists, 'Hailey', '1999 - 10/9/2022', 'Art/Design work', 'Caucasian Asian', 'I have a background in art, music, and design. I am attending BloomTech to further my skills.'));
...
function lotsOfArt(arrayParam) {
  ...
  for(let i = 0; i < arrayParam.length; i++) {
    if(arrayParam[i].paintings > 100) {
      filteredArray.push(arrayParam[i]['name']);
    }
  }
...
// console.log('Task 7', lostOfArt(artists, 100));
...
function artistInfo(arrayParam, artistsName){
  for(let i = 0; i < arrayParam.length; i++) {

    if(arrayParam[i]['name'] === artistsName) {
    return arrayParam[i].bio;
    }
  }
}

// console.log('Task 8', artistInfo(artists, 0));
...
function artistByCountry(arrayParam, nationality){
  const filteredArray = [];

  for(let i = 0; i < arrayParam.length; i++) {
    if(arrayParam[i].nationality === nationality) {
      filteredArray.push(arrayParam[i]['name']);
    }
  }
